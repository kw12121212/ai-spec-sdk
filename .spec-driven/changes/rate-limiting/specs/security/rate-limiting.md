# security/rate-limiting.md (delta)
...
- GIVEN a key has already consumed its full token allowance (120 requests) within the current window
- WHEN the client sends another `POST /rpc` with that key
...
- THEN the connection is accepted normally (no 429)

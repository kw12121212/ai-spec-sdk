...
为 HTTP 传输层添加基于令牌桶算法的按 API Key 限流机制。携带非 `admin` API Key 的 `POST /rpc` 请求默认每分钟最多发起 120 次；超限时返回 HTTP 429 和 JSON-RPC 错误响应，并附带 `X-RateLimit-*` 响应头。`admin` 范围的 Key 绕过所有限流检查。
...
- 限流仅作用于 `POST /rpc` 端点上的已认证非 `admin` Key 请求
- 超限响应：HTTP 429 + JSON-RPC 错误体（code `-32029`）+ `X-RateLimit-Limit`、`X-RateLimit-Remaining`、`X-RateLimit-Reset` 响应头
- 未超限的已认证非 `admin` Key 请求返回 `X-RateLimit-Limit` 和 `X-RateLimit-Remaining` 响应头

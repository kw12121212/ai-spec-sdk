...
The SDK MUST emit a `provider.fallback.activated` event on the bridge event stream whenever the fallback chain advances due to a provider error.

#### Scenario: Event emitted on fallback
- GIVEN provider "primary" has `fallbackProviderIds: ["backup"]`
- AND "primary" throws on query
- WHEN the SDK falls back to "backup"
- THEN a `provider.fallback.activated` event is emitted with `{ fromProviderId: "primary", toProviderId: "backup", reason: "<error message>", sessionId: "<id>" }`

...
- [x] Unit test: `session.cancel` successfully cancels a running session
- [x] Unit test: `session.cancel` returns error when called on non-running session
- [x] Unit test: `session.cancel` returns `-32011` error for unknown session
- [x] Unit test: `timeoutMs` on `session.start` automatically cancels session after duration
- [x] Unit test: `timeoutMs` on `session.resume` automatically cancels session after duration
- [x] Unit test: `timeoutMs` < 1 returns `-32602` error
- [x] Unit test: timer is cleared when session completes normally before timeout
- [x] Unit test: audit log entries are written for cancellation and timeout (via session-store tests)
- [x] Unit test: cancellation metadata is included in `session.status`, `session.list`, and `session.export` responses
- [x] Unit test: `bridge.capabilities` includes `session.cancel`
- [x] Lint passes (`bun run lint`)
- [x] All existing unit tests pass unchanged (4 non-related tests failed: HooksStore)

## Verification

- [x] Verify `session.cancel` spec scenarios are covered by tests
- [x] Verify `timeoutMs` spec scenarios are covered by tests
- [x] Verify audit logging spec scenarios are covered by tests
- [x] Verify capabilities spec scenario is covered by tests
